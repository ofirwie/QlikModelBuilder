...
const ProjectSpec_1 = require("./types/ProjectSpec");
const dashboardUI_1 = require("./ui/dashboardUI");
 ...
 */
function convertToProjectSpec(result, sourceFile) {
    const project = (0, ProjectSpec_1.createEmptyProjectSpec)(sourceFile);
    // Convert tables
    project.tables = result.tables.map((tableDef) => {
        ...
        if (tableDef.tableType) {
            ...
            if (type === 'fact')
                tableType = 'Fact';
            else if (type === 'dimension' || type === 'dim')
                tableType = 'Dimension';
            else if (type === 'bridge')
                tableType = 'Bridge';
        }
            ...
            tableDef.relationships.forEach((rel, relIndex) => {
                ...
                if (!relationshipsMap.has(id)) {
                    ...
                    if (rel.type) {
                        ...
                        if (type.includes('one-to-one') || type === '1:1')
                            cardinality = '1:1';
                        else if (type.includes('many-to-one') || type === 'm:1')
                            cardinality = 'M:1';
                        else if (type.includes('many-to-many') || type === 'm:m')
                            cardinality = 'M:M';
                    }
    ...
    if (result.fullSpec) {
        ...
        if (result.fullSpec.semanticLayer?.calculations) {
            project.formulas = result.fullSpec.semanticLayer.calculations.map((calc, i) => ({
                name: `calc_${i}`,
                expression: calc,
        ...
        if (result.fullSpec.semanticLayer?.measures) {
            project.formulas.push(...result.fullSpec.semanticLayer.measures.map((m, i) => ({
                name: `measure_${i}`,
                expression: m,
                type: 'measure'
            })));
        ...
        if (result.fullSpec.dataQuality?.globalRules) {
            project.businessRules = result.fullSpec.dataQuality.globalRules.map((rule, i) => ({
                id: `rule_${i}`,
    ...
    result.tables.forEach(tableDef => {
        if (tableDef.incremental.enabled) {
            project.userSelections.incrementalConfig[tableDef.name] = {
                enabled: true,
                strategy: tableDef.incremental.strategy,
 ...
 */
class WizardPanel {
    static createOrShow(extensionUri, qlikApi) {
    ...
    constructor(panel, extensionUri, qlikApi) {
        this._disposables = [];
        this._panel = panel;
    ...
    async parseSpecFile(filePath) {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`נמצאו ${result.tables.length} טבלאות (${factCount} Fact, ${dimCount} Dim), ${relCount} קשרים`);
        }
        catch (err) {
    ...
    parseExcelSpec(filePath) {
        ...
        const tablesSheet = workbook.SheetNames.find(s => s.toLowerCase() === 'tables' ||
    ...
    parseCSVSpec(filePath) {
        ...
        const isDetailedFormat = headerRow.some(h => /^(table|tablename|טבלה|שם_טבלה)$/i.test(h.trim())) && headerRow.some(h => /^(field|fieldname|column|שדה|עמודה)$/i.test(h.trim()));
        if (isDetailedFormat) {
        ...
        if (Array.isArray(tableArray)) {
            for (const t of tableArray) {
                tables.push({
                    ...
                    fields: (t.fields || t.columns || []).map((f) => ({
     ...
     */
    async parseDocumentSpec(filePath) {
        const ext = path.extname(filePath).toLowerCase();
        const warnings = [];
        ...
        if (!supportedFormats.includes(ext) && !needsTextExtraction.includes(ext)) {
            vscode.window.showWarningMessage(`קבצי ${ext} לא נתמכים. השתמש ב-.docx, .pdf או Excel.`);
        ...
        try {
            ...
            const geminiKey = process.env.GEMINI_API_KEY || config.get('geminiApiKey');
            const claudeKey = process.env.ANTHROPIC_API_KEY || config.get('anthropicApiKey');
            // Determine which AI to use
            let aiChoice;
            if (geminiKey && claudeKey) {
                // Both keys available - ask user
                aiChoice = await vscode.window.showQuickPick(['Gemini (מומלץ - תומך בקבצים)', 'Claude'], { placeHolder: 'בחר AI לניתוח המסמך' });
            }
            else if (geminiKey) {
            ...
            if (!aiChoice) {
                ...
                const keyChoice = await vscode.window.showQuickPick(['Gemini API Key', 'Claude API Key'], { placeHolder: 'אין API Key זמין. בחר איזה להזין:' });
                if (!keyChoice) {
     ...
     */
    async parseWithGeminiAPI(filePath, apiKey, warnings) {
        const tables = [];
        try {
...
JSON OUTPUT:`;
            ...
            const fullSpec = JSON.parse(jsonMatch[0]);
            // Store full spec for later use (ETL generation)
            this._lastParsedSpec = fullSpec;
            ...
            if (tables.length > 0) {
                ...
                vscode.window.showInformationMessage(`✅ Gemini חילץ "${projectName}": ${tables.length} טבלאות, ${totalFields} שדות`);
            }
            else {
                warnings.push('Gemini לא מצא טבלאות במסמך');
            }
            return { tables, source: filePath, warnings, fullSpec: fullSpec };
        }
        catch (err) {
     ...
     */
    async parseWithClaudeAPI(filePath, apiKey, warnings) {
        const tables = [];
        try {
...
JSON OUTPUT:`;
            ...
            if (tables.length > 0) {
                const totalFields = tables.reduce((sum, t) => sum + t.fields.length, 0);
                const projectName = fullSpec.projectMetadata?.name || 'Unknown';
                vscode.window.showInformationMessage(`✅ Claude חילץ "${projectName}": ${tables.length} טבלאות, ${totalFields} שדות`);
            }
            else {
...
<head>
  ...
  <style>
    ${(0, dashboardUI_1.getDashboardStyles)()}
  </style>
</head>
<body>
  ...
  <script nonce="${nonce}">
    ${(0, dashboardUI_1.getDashboardScript)()}
  </script>
</body>
</html>`;
    }
